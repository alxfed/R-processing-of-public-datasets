...
qplot(fdf$Measurement.Timestamp, fdf$Air.Temperature, color = I("magenta"), 
      size = I(.5), alpha = I(1/2))
```


As we can see ggplo2 'understands' our Date/Time format. Interestingly enough there is a gap in data in May.

```{r}
qplot(sdf$Measurement.Timestamp, sdf$Air.Temperature, color = I("red"), 
      size = I(.5), alpha = I(1/2))
```

```{r}
qplot(odf$Measurement.Timestamp, odf$Air.Temperature, color = I("green"), 
      size = I(.5), alpha = I(1/2))
```
